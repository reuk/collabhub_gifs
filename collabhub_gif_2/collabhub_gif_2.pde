...
ArrayList<Circle> get_circles() {
  ...
  for (int i = 0; i != STEPS_0; ++i) {
    ...
    circle.add(new Circle(0, r0, new PVector(sin(ANGLE) * r1, cos(ANGLE) * r1)));
  }
  ...
  for (int i = 0; i != STEPS_1; ++i) {
    ...
    circle.add(new BounceCircle(0.2, base_rad, base_rad, 0.5 + i * 0.025, 0.2, ANGLE));
  }

  for (int i = 0; i != STEPS_0; ++i) {
    final float ANGLE = i * TWO_PI / STEPS_0 + TWO_PI / (2 * STEPS_0);
    final float r1 = base_rad * ratio;
    circle.add(new Circle(0.6, base_rad, new PVector(sin(ANGLE) * r1, cos(ANGLE) * r1)));
  }
...
class BounceCircle extends Circle {
  ...
  void update(float TIME) {
    ...
    float current_amp = map(cos(ALTERED_TIME * TWO_PI), -1, 1, 20, 0);
    _current = new PVector(sin(_angle) * (_distance + current_amp), cos(_angle) * (_distance + current_amp));
  }
